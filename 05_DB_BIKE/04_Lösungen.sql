...
*/
	...
	-- Query
	SELECT sum(Anzahl) AS "Reservierungen (kum.)" 
	FROM Reservierung;
...
*/
	...
	SELECT Artnr AS Artikelnummer, (Bestand - Mindbest - Reserviert) AS Berechnung
		FROM Lager
		WHERE (Bestand - Mindbest - Reserviert) < 3
		ORDER BY Berechnung DESC
	; 
	-- Tab. Lager + Artikel m. INNER JOIN + Bedingung
	SELECT Artnr AS Artikelnummer, Bezeichnung, (Bestand - Mindbest - Reserviert) AS Berechnung
		FROM Lager 
...
*/
	...
	SELECT * FROM Artikel;
	-- Query
	SELECT Artnr, Bezeichnung, count(Artnr) AS Teile
